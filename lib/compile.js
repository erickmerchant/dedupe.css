...
export const compile = async (args) => {
  ...
  const addToMap = (namespace, root, name, id) => {
    ...
    if (vmap[namespace][root] == null) {
      vmap[namespace][root] = new Set()
    }

    if (root !== name) {
      vmap[namespace][root].add(name)
    }
  ...
  while (map[defineID] != null) {
    defineID = `define${++i}`
  }

  if (args['--dev']) {
    output.js.result += `import {define as ${defineID}} from '@erickmerchant/css'\n`
  }
