...
import {createGetClassName} from './create-get-class-name.js'
...
export const compile = async (args) => {
  ...
  const addToMap = (namespace, root, name, id) => {
  ...
  const getClassName = createGetClassName(args['--prefix'] ?? '')

  const multiGenerator = createMultiGenerator(getClassName, addToMap)

  const singleGenerator = createSingleGenerator(getClassName, addToMap)
