    ...
    if (multis.length) {
      for (const multi of multis) {
        ...
        if (
          ...
          const nameSuffixes = await model.selectNameSuffixes(
            multi.atruleID,
            multi.prop,
            multi.value
          )
          ...
          const excludedIDs = nameSuffixes.map(({id}) => id)

          for (const nameSuffix of nameSuffixes) {
            if (prevSuffix !== nameSuffix.suffix) {
              id = getUniqueID()

              selectors.push(
                `${Array(nameSuffix.repeat).fill(`.${id}`).join('')}${
                  nameSuffix.suffix
                }`
              )
            ...
            addToMap(nameSuffix.namespace, nameSuffix.name, id)

            prevSuffix = nameSuffix.suffix
          }
