...
export const compile = async (args) => {
        ...
        for (const item of shorthandItems) {
          if (
            ...
            if (item.suffix) {
              ...
              item.suffix = selectorTokenizer.stringify(selectorNodes[0])
    ...
    if (singles.length) {
      ...
      for (const single of singles) {
        ...
        if (
          ...
          single.suffix = selectorTokenizer.stringify(selectorNodes[0])
  ...
  if (!args['--dev']) {
    ...
    while (map[classyFactoryId] != null) {
      classyFactoryId = `_${++i}`
    }

    output.js.write(`const ${classyFactoryId} = () => {
      const classy = (prop, obj = {}) => {
        let classes = classy[prop]

        for (let i = 0, keys = Object.keys(obj); i < keys.length; i++) {
          if (obj[keys[i]]) {
            classes += ' ' + classy[keys[i]]
          }
        }
  ...
  for (const namespace of Object.keys(map)) {
    ...
    if (args['--dev']) {
      output.js.write(`export const ${namespace} = new Proxy(
        (() => {
          const classy = (prop, obj = {}) => {
            if (!{}.hasOwnProperty.call(classy, prop)) {
          ...
          ${Object.entries(map[namespace])
            .map(
              ([key, value]) =>
                `Object.defineProperty(classy, ${JSON.stringify(key)}, {
                get() {
      ...
      output.js.write(
        `export const ${namespace} = Object.assign(${classyFactoryId}(), ${JSON.stringify(
          map[namespace]
        )})\n`
      )
    }
