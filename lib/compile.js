...
export const compile = async (args) => {
  ...
  const buildCSS = async (searchID) => {
    ...
    const singles = await db.all(sql.selectSingles, searchID)
    ...
    const multis = await db.all(sql.selectMultiples, searchID)
    ...
    const atrules = await db.all(sql.atrules, searchID)
  ...
  const rows = await db.all(sql.shortLong)

  for (const row of rows) {
    console.warn(`${row.shortProp} found with ${row.longProp} for ${row.name}`)
  }
