...
export const insertDecl = async (atruleID, nameID, suffix, prop, value) => {
  ...
  await run(
    `
      INSERT INTO decl (atruleID, nameID, suffix, prop, value, weight, repeat)
      VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT (atruleID, nameID, suffix, prop)
        DO UPDATE SET value = ?, repeat = ?
    `,
    atruleID,
    nameID,
    suffix,
...
export const selectSingles = (searchID) =>
  all(
    `
      SELECT name, nameID, namespace, prop, suffix, value
      FROM decl
...
export const selectMultiples = (searchID) =>
  all(
    `
      SELECT atruleID, prop, value, GROUP_CONCAT(DISTINCT nameID) as nameIDs, GROUP_CONCAT(DISTINCT suffix || repeat) as suffixes
      FROM decl
      WHERE atruleID = ?
      GROUP BY atruleID, prop, value
        HAVING COUNT(id) > 1
...
export const selectNameSuffixes = (atruleID, prop, value) =>
  all(
    `
      SELECT decl.id as id, namespace, name, suffix, nameID, repeat
      FROM decl
        LEFT JOIN name ON decl.nameID = name.id
      WHERE atruleID = ?
        AND prop = ?
